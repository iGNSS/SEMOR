	...
	while(1){ //exit when return
		...
		if (flags == 7)
		{
			...
			if(first_read){
				...
				imu_sec = (double)(((tv.tv_sec+LEAP_SECONDS-GPS_EPOCH))%(7*24*3600))+(tv.tv_usec / 1000000.0);
				first_read = 0;
			}
			else{
				imu_sec += (stamp-last_stamp) * 2.5e-5;
			}

			struct timeval tvv;
            gettimeofday(&tvv, NULL);
            int week1 = ((tvv.tv_sec+LEAP_SECONDS-GPS_EPOCH))/(7*24*3600);
            double sec1 = (double)(((tvv.tv_sec+LEAP_SECONDS-GPS_EPOCH))%(7*24*3600))+(tvv.tv_usec / 1000000.0);
			/*nsamples += 1;
			tot_acc_x += ax*acclUnits*gravity;
			tot_acc_y += ay*acclUnits*gravity;
			tot_acc_z += az*acclUnits*gravity;*/
			sprintf(line, "%d %lf %7.4f %7.4f %7.4f %8.3f %8.3f %8.3f | %10u %d | (preso: %lf)", week,
				imu_sec,
				/*ax,ay,az, gx,gy,gz,*/
        
        /* HUGO */
				ax*acclUnits*gravity/**0.0+0.0192*//*-avg_acc_x*/, ay*acclUnits*gravity/**0.0+0.0239*//*-avg_acc_y*/, az*acclUnits*gravity/**0.0-9.9168*//*-avg_acc_z*/,
				gx*gyroUnits/**0.0-0.314961*/, gy*gyroUnits/**0.0-0.280035*/, gz*gyroUnits/**0.0+0.437949*/, stamp, n, sec1);
			  /* HUGO END */
			last_stamp = stamp;
